          ...
          {orders.map((row) => (
            <tr key={row.id} className="hover:bg-slate-50/60 transition-colors">
              <td className="px-8 py-4">
                <span className="text-[16px] font-bold text-slate-900 font-mono">#{row.orderId || row.id}</span>
              </td>
              <td className="px-5 py-4">
                <p className="text-[16px] font-semibold text-slate-700">{row.customer || row.customerName || row.shippingAddress?.name || "Customer"}</p>
              </td>
              <td className="px-5 py-4">
                <p className="text-[16px] font-bold text-slate-900">₹{Number(row.total || 0).toLocaleString()}</p>
              </td>
              <td className="px-8 py-4 text-right">
                <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-[16px] font-bold uppercase tracking-wide border ${statusBadgeClasses(row.status || "Pending")}`}>
                  {row.status || "Pending"}
                </span>
              </td>
            </tr>
    ...
    <div className="lg:hidden divide-y divide-slate-100">
      {orders.map((row) => (
        <div key={row.id} className="p-5 flex items-center justify-between hover:bg-slate-50 transition-colors">
          ...
          <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-[16px] font-bold uppercase tracking-wide border ${statusBadgeClasses(row.status)}`}>
            {row.status || "Pending"}
          </span>
        </div>
      ))}
