...
const Stat = ({ label, value, note, icon: Icon, tint, positive = true }) => (
  <article className="rounded-2xl border border-slate-100 bg-white p-5 shadow-[0_3px_14px_rgba(37,17,24,.035)]">
    <div className="flex items-start justify-between"><div><p className="text-[14px] font-semibold text-slate-500">{label}</p><p className="mt-2 text-[25px] font-bold tracking-tight text-slate-900">{value}</p></div><span className={`grid h-11 w-11 place-items-center rounded-full ${tint}`}><Icon size={20} strokeWidth={1.8} /></span></div>
    <p className="mt-3 flex items-center gap-1 text-[14px] text-slate-500"><ArrowUpRight size={12} className={positive ? "text-emerald-500" : "rotate-90 text-rose-500"} /><strong className={positive ? "text-emerald-600" : "text-rose-600"}>{note}</strong> vs previous period</p>
  </article>
);
